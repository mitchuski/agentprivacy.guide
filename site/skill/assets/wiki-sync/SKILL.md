...
```
guide.guide.agentprivacy.ai  ← the hub (build-guide-host.js): index · federation-map · distribution-gates · contribute
   ├─ skill.guide.agentprivacy.ai     ← agentprivacy-skills-v5
   ├─ tomes.guide.agentprivacy.ai     ← cityofmages/tomes
   ├─ research.guide.agentprivacy.ai  ← agentprivacy-docs
   ├─ atlas.guide.agentprivacy.ai     ← spellweb/src/data  (the graph — the hyperlink backbone)
   └─ library.guide.agentprivacy.ai   ← agentprivacy_master/public  (the 5 narrative spellbooks)
```
...
| **Dual-agent harness** — core (loop·trusts·ground rules·contract) + 7 seats + specialisation + 10 harness paths + workflows/runtimes + field-guide toy | `~/dual-agent-harness` | **harness** (NEW site) | `build-harness.js` | ✅ (chronicles/ · universe/ · runs/ · instance ledgers 🔒 — numbers cited from frontier.json, never paged) |
| **DTG research KB** — decision baseline (§-split) + explorations + lab NOTES + 2 chronicles | `~/dtgwg-cred-spec-main_mage` via its own `tools/build-kb.mjs` (manifest-first + secrets gate; emits `kb/fedwiki/`) | **dtg** (NEW site) | external: `node tools/build-kb.mjs` then wire pages (filename = slugify(title)) | ✅ 77 (vendored spec 58 files NEVER projected; 2 chronicle slugs = manifested circulating, leak-scan false positives) |
...
curl -s -H "Host: <host>.localhost" http://localhost/system/sitemap.json | grep -o '"slug"' | wc -l
...
curl -s -H "Host: <host>.localhost" http://localhost/system/sitemap.json | grep -oE '"slug":"[^"]+"' | grep -iE "chronicle|draft|plan|audit|candidate|presync|\.bak"
```
