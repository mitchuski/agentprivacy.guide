...
| T3 shared root | **topological**: the boot preamble gives every seat the same three documents and its own card, nothing more; the measure seat's numbers are the only shared context, and they are frontier state | `AGENTS.md` (via `CLAUDE.md`) · engine boot preamble |
